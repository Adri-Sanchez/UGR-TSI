(define (domain P2)
     ...
     (:action COGER
          ...
          :precondition (and (posicion ?r ?z) (posicion ?i ?z) (manovacia ?r))
          :effect (and (not (posicion ?i ?z)) (not (manovacia ?r)) (tengo ?r ?i) 				 (enMano ?r ?i) 
          		  )
     )
     ...
     (:action SOLTAR
          ...
          :effect (and (not (tengo ?r ?o)) (not (enMano ?r ?o)) (posicion ?o ?z) 				 (manovacia ?r)
          		  ) 
     )
     
     (:action ENTREGAR
          :parameters (?r - robot ?o - objeto ?z - zona ?p - personaje)
          :precondition (and (posicion ?r ?z) (posicion ?p ?z) (tengo ?r ?o) 						   (enMano ?r ?o) 
                        )
          :effect (and (not (tengo ?r ?o)) (not (enMano ?r ?o)) (tiene ?p ?o) 
                       (manovacia ?r) (increase (totalPuntos) (puntos ?p ?o))
                  )
     )
    ...
    (:action GUARDAR;;
	    ...
	    :effect (and (not (enMano ?r ?o)) (decrease (mochila ?r) 1) (manovacia ?r))
    )

	(:action SACAR;;
	    :parameters (?r - robot ?o - objeto)
	    :precondition(and (manovacia ?r) (tengo ?r ?o))
	    :effect(and (not (manovacia ?r)) (increase(mochila ?r) 1) (enMano ?r ?o))
    )
